# Authors and Citation

## Authors

- **[David Gohel](https://www.ardata.fr)**. Author, maintainer.

- **Hadley Wickham**. Author.

- **Lionel Henry**. Author.

- **Jeroen Ooms**. Author. [](https://orcid.org/0000-0002-4035-0289)

- **Yixuan Qiu**. Contributor.

- **R Core Team**. Copyright holder.  
  Cairo code from X11 device

- **ArData**. Copyright holder.

- **RStudio**. Copyright holder.

## Citation

Source:
[`DESCRIPTION`](https://github.com/davidgohel/gdtools/blob/master/DESCRIPTION)

Gohel D, Wickham H, Henry L, Ooms J (2026). *gdtools: Utilities for
Graphical Rendering and Fonts Management*. R package version 0.4.5.003,
<https://davidgohel.github.io/gdtools/>.

    @Manual{,
      title = {gdtools: Utilities for Graphical Rendering and Fonts Management},
      author = {David Gohel and Hadley Wickham and Lionel Henry and Jeroen Ooms},
      year = {2026},
      note = {R package version 0.4.5.003},
      url = {https://davidgohel.github.io/gdtools/},
    }
